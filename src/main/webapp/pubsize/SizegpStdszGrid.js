Ext.define('y.pubsize.SizegpStdszGrid',{
...
//		}
	  ...
	  me.store=Ext.create('Ext.data.Store',{
			...
			proxy:{
				type: 'ajax',
			    url : Ext.ContextPath+'/pubSize/querySizegpStdsz.do',
...
//		child.set("id",null);
		...
		var stdszSelGrid=Ext.create('y.pubsize.StdszSelGrid',{
			...
			listeners:{
				itemdblclick:function(view, record, item, index, e, eOpts){
					Ext.Ajax.request({
						url:Ext.ContextPath+'/pubSize/createSizegpStdsz.do',
						params:{
    ...
    onDelete:function(){
		...
		Ext.Msg.confirm("删除",'确定要删除吗?', function(btn, text){
			if (btn == 'yes'){
				Ext.Ajax.request({
						url:Ext.ContextPath+'/pubSize/deleteSizegpStdsz.do',
						params:{
							szbrad:me.getStore().getProxy().extraParams.szbrad,
							szclno:me.getStore().getProxy().extraParams.szclno,
							sizeno:record.get("sizeno")
